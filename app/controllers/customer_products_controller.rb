class CustomerProductsController < ApplicationController
  def index
    ...
    if params[:customer_id]
      ...
      if(@sort_by === "sales")
        @products = @product.customer_products.order(:sales_year => :desc)
      elsif @sort_by === "growth"
        @products = @product.customer_products.order(:growth => :desc)
      elsif @sort_by === "name"
        @products = @product.customer_products.order(:name)
      end
    end
